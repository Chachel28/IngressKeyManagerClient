//API's URLs
//String baseUrl = "10.0.2.2:8080";
String baseUrl = "192.168.1.21:8080";
//String baseUrl = "192.168.43.218:8080";

String endpointUsers = "api/v1/user";
String endpointLogin = "api/v1/login";
String endpointOutdated = "api/v1/isReswueOutdated";
String endpointReswueURL = "api/v1/reswueURL";
String endpointReswueToken = "api/v1/createReswueToken";
String endpointOperations = "api/v1/operations/";
String endpointPortalKeys = "api/v1/portal/keys/";
String endpointRoutes = "api/v1/route/";
String endpointRoutesStops = "api/v1/route/stop";
String endpointRoutesKeys = "api/v1/route/key";

//Keys for shared preferences
String idKey = "ID_TOKEN";
String usernameKey = "USER_TOKEN";
String userImageKey = "IMAGE_TOKEN";
String emailKey = "EMAIL_TOKEN";
String apiTokenKey = "API_TOKEN";
String reswueTokenKey = "RESWUE_TOKEN";
String reswueRefreshTokenKey = "RESWUE_REFRESH_TOKEN";
String codeKey = "CODE_KEY";